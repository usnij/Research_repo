...
Insta360 X5로 촬영한 equirectangular(정방형) 이미지에서, 여러 yaw/pitch 조합의 perspective view 이미지를 추출했다. 이 과정에서 equirectangular → rectilinear(pinhole) 변환이 이루어지므로, 결과 이미지에는 렌즈 왜곡이 없다.
...
- **총 15장**의 perspective view 이미지 생성
- 각 이미지는 왜곡 없는 핀홀 카메라 이미지에 해당
...
|------|-----------|------|-------|
| **3DGS** | **41.75** | **0.9883** | **0.0582** |
| **3DGUT** | 26.64 | 0.8976 | 0.1498 |
| **차이** | **-15.11 dB** | **-0.0907** | **+0.0916** |

3DGS가 **모든 메트릭에서 압도적으로 우세**하다. 특히 PSNR 차이가 15.11dB로 매우 크며, 이는 육안으로도 명확히 구분 가능한 수준이다.

### 5.3 이전 실험과의 비교

[Practice v1](https://github.com/usnij/Research_repo/blob/main/Study/3DGUT_Practice_v1.md)에서도 동일한 패턴이 관찰되었다:

| 실험 | 방법 | PSNR (dB) | SSIM | LPIPS |
|------|------|-----------|------|-------|
| v1 (아이폰, 135장) | 3DGS | **37.29** | **0.9815** | **0.0559** |
| v1 (아이폰, 135장) | 3DGUT | 31.59 | 0.9633 | 0.0850 |
| **v4 (360→perspective, 15장)** | **3DGS** | **41.75** | **0.9883** | **0.0582** |
| **v4 (360→perspective, 15장)** | **3DGUT** | **26.64** | **0.8976** | **0.1498** |

v4에서 PSNR 격차가 v1(5.7dB)보다 훨씬 커진 이유는 **15장이라는 적은 이미지 수**가 3DGUT에 더 불리하게 작용했기 때문으로 보인다. 3DGS는 적은 데이터에서도 높은 복원력을 보이는 반면, 3DGUT의 UT 기반 투영은 데이터가 부족할수록 수렴이 어려워질 수 있다.
...
15장이라는 적은 수의 학습 이미지도 품질 저하의 원인이 될 수 있다. 360도 카메라에서 더 많은 방향의 perspective view를 추출하면 개선될 여지가 있다.
