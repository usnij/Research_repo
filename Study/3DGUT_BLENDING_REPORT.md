...
depth = particleMean.x * viewMatrix[0][2]
...
같은 Gaussian이라도 어떤 ray(픽셀)에서 보느냐에 따라 hitT는 달라진다. 그런데 globalDepth는 그 프레임에서 카메라 기준 하나의 값으로 고정돼.
...
3. 새 (tile_idx, depth_key) 키로 radix sort 재실행
    ...
    렌더링:
      ...
      4. 두 임계값 모두 통과?
         → hitT 계산
         → (idx, hitT, alpha) K-buffer에 insert
         → 아니면 skip
