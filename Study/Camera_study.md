...
## Camera pose estimation
  - 이미지가 월드좌표계에서 어디에 있고, 어디를 보고 있는지(R, t)를 추정하는 것
  - 이는 보통 카메라의 외부 변수로 표현한다.

- 3D 점들의 월드좌표계상에서의 위치를 이미지와 함께 입력으로 받아 R,t를 출력하는 PnP방식이 있다.
  - 이는 이미 3D 구조를 알고 있는 상황에서 들어온 이미지에 대해 어디서 찍혔는지 추정하게 된다. 
...
- SfM
  ...
  1. Feature Extration
    - 각 이미지마다 feature를 추출한다. 이는 SIFT 알고리즘으로 주로 진행된다.  
      - SIFT는 이미지의 크기나 회전에 불변하다는 특징이 있다. 

  2. Matching
  - 이미지 쌍 간 descriptor 비교
  3. 기하 검증
  - Feature들이 기하적으로 말이 되는지 확인하는 단계
  - RANSAC으로 잘못된 매칭 제거
  - 에피폴라 제약 사용
