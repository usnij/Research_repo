...
- $\text{erf\_tot}$ : ray-ellipsoid 교차 구간 $[t_1, t_2]$의 적분 비율 $= \operatorname{erf}(\sqrt{\text{disc}/2})$
- $\sigma_{0,\text{med}}$ : erf_tot로 변조된 광학 깊이(optical depth)
...
- $\text{erf\_tot} = \operatorname{erf}(\sqrt{\text{disc}/2})$ : $[t_1, t_2]$ 구간이 전체 가우시안 적분 중 차지하는 비율
...
$$\text{erf\_tot} = \operatorname{erf}(v),\quad v = \sqrt{\text{disc}/2} \quad\Rightarrow\quad \frac{\partial\mathcal{L}}{\partial\,\text{disc}}\bigg|_{\text{erf}} = \frac{\partial\mathcal{L}}{\partial\,\text{erf\_tot}} \cdot \frac{2}{\sqrt{\pi}} \cdot \frac{e^{-v^2}}{4v}$$
