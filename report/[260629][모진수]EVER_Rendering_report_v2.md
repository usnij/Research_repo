...
> GUT 중경 이미지에서 줄기 영역의 검정 선 artifact 확인 가능 — SH2의 방향별 색상 표현력 부족으로 특정 시점에서 내부 줄기 Gaussian의 색상 평가값이 0으로 클램핑됨.
...
크래시 수정 후 30k 학습은 완료됐지만, 테스트 200장 중 **58장(29%)이 완전 검정 렌더**로 출력됨. 평균 PSNR 28.0 dB는 NeRF synthetic의 검정 배경과 검정 렌더가 pixel-wise로 부분 일치해 실제보다 높게 측정된 misleading한 수치.
...
![GUT SH2 검정 선 artifact](report_image_모진수/260629/gut_middle_annotated.png)
