...
$$
C = \int_0^\infty c_{\mathbf{r}}(t)\,\sigma_{\mathbf{r}}(t)\exp\left(-\int_0^t \sigma_{\mathbf{r}}(s)\,ds\right)dt
\quad \cdots (1)
$$
...
$$
\sigma_i = \sum_{k=1}^{i} \Delta\sigma_k, \qquad \mathbf{c}_i = \frac{1}{\sigma_i}\hat{\mathbf{c}}_i = \frac{1}{\sigma_i}\sum_{k=1}^{i}\Delta\sigma_k\,\Delta\mathbf{c}_k
\quad \cdots (2)
$$
...
$$
\text{Contribution of segment } i = \mathbf{c}_i\bigl(1 - \exp(-\sigma_i\,\Delta t_i)\bigr)
\quad \cdots (3)
$$
...
$$
C = \sum_{i=1}^{N} \mathbf{c}_i\bigl(1 - \exp(-\sigma_i\,\Delta t_i)\bigr)\prod_{j=1}^{i-1}\exp(-\sigma_j\,\Delta t_j)
\quad \cdots (4)
$$
...
$$
0.99 < 1 - \exp\bigl(-\sigma(\alpha_k) \cdot \max(s_{kx}, s_{ky}, s_{kz})\bigr)
\quad \cdots (6)
$$
...
$$
\text{stopgrad}(1-\alpha)\bigl(\max(s_x,s_y,s_z) - \min(s_x,s_y,s_z)\bigr)
\quad \cdots (7)
$$
...
### 5.6 Zip-NeRF 씬별 상세 결과 (논문 Supplementary)

Zip-NeRF 데이터셋은 Barron et al.(ICCV 2023)이 공개한 4개의 대규모 실외 도시 씬(alameda, berlin, london, nyc)으로 구성된다. EVER 논문 본문 Table 1에는 4씬 평균만 제시되며, 씬별 수치는 Supplementary에 수록되어 있다.

> **주의**: 아래 결과는 모두 표준 핀홀(undistorted) 이미지 기준이다. 3DGS·StopThePop이 fisheye를 지원하지 않으므로, 공정 비교를 위해 전 방법이 동일하게 보정된 이미지를 사용한다.
...
| **EVER (Ours)** | **27.24** | **27.93** | **24.72** | **26.49** | **26.60** |
...
| **EVER (Ours)** | **.371** | **.337** | **.389** | **.374** | **.368** |
| ZipNeRF (offline) | .378 | .331 | .387 | .360 | .364 |

**분석**: SSIM 기준으로 EVER는 전 씬에서 offline 방법인 ZipNeRF를 상회한다. LPIPS에서는 nyc에서 ZipNeRF와 동등 수준(.337 vs .331)이며, berlin에서는 오히려 EVER가 더 낮다(.371 vs .378). 복잡한 도심 씬일수록 exact blending의 효과가 두드러진다.

### 5.7 Fisheye 원본 데이터 직접 비교 실험 (EVER vs 3DGUT)

위 5.6의 결과는 undistorted 이미지 기준이다. EVER는 ray tracing 기반이므로 fisheye 원본 이미지를 왜곡 보정 없이 직접 학습·평가할 수 있다. 이 이점을 검증하기 위해 동일 씬을 **OPENCV_FISHEYE 원본 이미지** 그대로 EVER와 3DGUT으로 각각 학습한 결과를 비교한다. 3DGS·StopThePop은 fisheye를 지원하지 않으므로 비교 대상에서 제외된다.

**실험 설정**

| 항목 | 값 |
|------|-----|
| 데이터 | alameda, london, nyc (OPENCV_FISHEYE 원본) |
| 해상도 | downsample_factor=4 |
| 학습 이터레이션 | 30,000 |
| train/test 분할 | test_split_interval=8 |
...
| 씬 | 지표 | EVER | 3DGUT |
|----|------|------|-------|
| **alameda** | PSNR↑ | **21.288** | 20.733 |
| | SSIM↑ | **0.772** | 0.736 |
| | LPIPS↓ | **0.286** | 0.368 |
| | 학습 시간 | 74.2 min | **15.2 min** |
| | 추론 속도 | ~323 ms/f | **4.68 ms/f** |
| **london** | PSNR↑ | **24.131** | 23.718 |
| | SSIM↑ | **0.801** | 0.777 |
| | LPIPS↓ | **0.331** | 0.417 |
| | 학습 시간 | 65.4 min | **13.7 min** |
| **nyc** | PSNR↑ | **27.593** | 25.850 |
| | SSIM↑ | **0.895** | 0.860 |
| | LPIPS↓ | **0.194** | 0.295 |
| | 학습 시간 | 75.8 min | **14.5 min** |

**전체 평균 요약**

| 항목 | EVER | 3DGUT |
|------|------|-------|
| PSNR 평균↑ | **24.337** | 23.434 (+0.90 dB 우위) |
| SSIM 평균↑ | **0.823** | 0.791 |
| LPIPS 평균↓ | **0.270** | 0.360 |
| 학습 시간 평균 | 71.8 min | **14.5 min (~5배 빠름)** |
| 추론 속도 | ~323 ms/f | **~4.4 ms/f (~70배 빠름)** |

**해석**: 품질 전 지표에서 EVER가 일관되게 우세하며, 복잡한 도심 씬(nyc)일수록 격차가 커진다(PSNR +1.74 dB). 단 속도 면에서는 3DGUT이 학습 5배·추론 70배 빠르다. fisheye 지원 자체는 두 방법 모두 가능하며, 오프라인 품질 우선 시 EVER, 실시간 필요 시 3DGUT이 유리하다.

> 참고: 3DGUT 저자들도 논문 Conclusion에서 overlap 처리 한계를 직접 인정하며 EVER를 향후 방향으로 명시하였다.
> *"Approaches such as EVER may offer promising directions for addressing this limitation."* — 3DGUT (CVPR 2025)
