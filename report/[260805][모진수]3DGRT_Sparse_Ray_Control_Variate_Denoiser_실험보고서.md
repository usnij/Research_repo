...
이 최적화는 추론 전용이다. 학습 중에는 Gaussian이 매 step 갱신되므로 쓸 수 없다. 다만 1/64 sparse ray가 실제로 건드리는 Gaussian은 전체의 **7.9~10.6%** 뿐이므로(3.3.2절), 학습에서는 필요한 부분만 activation하는 별도 최적화가 가능하다. 본 보고서 범위 밖이다.

### 3.2 총 작업량은 정확히 ray 수에 비례한다

먼저 분모를 고정해야 한다. Tracer의 작업 단위는 ray가 아니라 **hit**이다. ray 하나가 장면을 통과하며 응답이 임계값을 넘는 Gaussian을 만날 때마다 SH를 평가하고 투과율을 누적한다. 따라서

$$W = N_{\text{ray}} \times h$$

여기서 $W$ 는 총 작업량(hit 수), $N_{\text{ray}}$ 는 ray 수, $h$ 는 ray당 hit count다.

이고, $h$가 ray 수에 따라 변한다면 "ray를 줄였는데 왜 안 빨라지나"라는 질문 자체가 성립하지 않는다. 실측에서 $h$는 **30.4로 완전히 일정하다.**
...
당연한 결과다. 어느 ray든 같은 장면을 지나가므로 마주치는 Gaussian 수의 기댓값이 같다. 요점은 이것이 **통제 변인**이라는 데 있다 — ray를 1/64로 줄이면 총 작업량도 **정확히 1/64**로 줄어든다. 그러므로 시간이 1/64로 줄지 않는다면 원인은 작업량이 아니라 **작업 하나를 처리하는 효율**에 있다.

### 3.3 그런데 시간은 비례하지 않는다
...
3.2절에서 hit당 작업량이 균질함을 확인했으므로, **ns/hit이 0.83에서 4.12로 5.0배 나빠졌다는 것은 동일한 연산을 5배 느리게 처리하고 있다는 뜻**이다. 이것이 "1/64가 64배가 아니라 12.9배"의 전부다.

#### 고정 오버헤드 모형은 맞지 않는다

가장 흔한 설명은 상수항이다. $t = a + bN$ 으로 보면 상위 두 점에서 $b = 22.6$ ns/ray, $a = 4.4$ ms가 나온다. 이 모형은 25,350 ray에서 **4.97 ms**를 예측하지만 실측은 **3.175 ms**다. 상수항이 실제로 있다면 저 ray 수에서 이미 전액이 청구되어야 하는데 그렇지 않다. 반대 방향으로도 어긋난다 — 117 ray에서 모형은 4.4 ms를, 실측은 1.625 ms를 준다.

즉 **비용의 일부가 ray 수에 따라 나타났다 사라지는** 형태이며, 이는 상수항으로 표현되지 않는다.

#### 지연-처리량 전이로 설명된다

BVH 순회는 의존적 메모리 접근의 연쇄다. 노드를 읽어야 다음에 어느 노드를 읽을지 알 수 있으므로, ray 하나의 순회 시간은 대기 시간(latency)의 합이다. 이 대기는 **다른 ray의 작업으로만 가려진다.**

$$t \approx \max\bigl(L,\; N_{\text{ray}} \cdot h \cdot c_{\infty}\bigr)$$
...
| 구간 | rays | 거동 | 해석 |
|---|---|---|---|
| 하단 | 117 ~ 425 | 1.625 → 1.657 ms, **거의 평탄** | $L$ 지배. ray를 3.6배 늘려도 시간이 안 늘어남 |
| 중단 | 1,617 ~ 101,400 | 매끄러운 전이 | 두 항이 비슷한 크기 |
| 상단 | 405,600 ~ 1.6M | 4배 ray → 3.03배 시간 | $c_{\infty}$ 지배 |

**1/64(25,350 ray)는 전이 구간 한가운데에 있다.** 포화 처리량의 5배를 내지만 완전한 지연 노출($L$)보다는 싸다. 여기서 ray를 더 줄여도 시간은 $L$에 수렴할 뿐이라 얻을 것이 없다 — 실제로 25,350 → 117로 216배 줄여도 3.175 → 1.625 ms, 2배도 안 빨라진다.

이 해석은 정합적이지만 완전히 검증되지는 않았다. 하단 평탄값 1.6 ms를 ray 하나의 순회 지연으로 환산하면 13.9 µs/ray인데, 이는 hit 30개의 의존적 접근(약 300 ns × 30 ≈ 9 µs)과 같은 자릿수이나 정확히 일치하지는 않는다. 노드 순회가 accepted hit보다 훨씬 많다는 점을 고려하면 방향은 맞다.

### 3.3.1 전이 구간에서 무엇이 효율을 결정하는가

$L$이 노출되는 정도는 **동시에 진행 가능한 독립 작업의 양**으로 정해진다. 이를 좌우하는 후보가 둘이고, 서로 독립이므로 분리해서 측정했다.
...
**(a) ray 수 (occupancy).** 희소성을 유지한 채 ray만 4배 늘리면(1뷰 → 4뷰) 125 → 70.8 ns/ray, **1.8배**. 날아다니는 ray가 많아져 서로의 지연을 가려준다.

**(b) ray coherence.** ray 수를 그대로 두고 배치만 연속으로 바꾸면 125 → 37.7 ns/ray, **3.3배**. 이쪽이 더 크다. Dense 렌더링은 워프 내 인접 thread가 인접 픽셀을 쏘므로 **같은 BVH 노드를 같은 시점에 읽는다** — 한 번의 메모리 트랜잭션이 여러 ray를 서비스한다(재사용 164.8회). 1/64는 인접 thread가 8픽셀씩 떨어져 서로 다른 경로로 발산하고 재사용이 4.9회로 무너진다.

두 요인이 **대체 가능하지 않다는 것**이 중요하다. 제어 변량(control variate) $B$ 경로(405,600 ray, 연속)와 sparse 경로(25,350 ray, 희소)를 하나의 launch로 합치면 총 ray가 17배가 되지만 절감은 **0.346 ms**에 그쳤다(17.539 → 17.193). 융합 launch 안에서 sparse ray의 한계 비용은 113 ns/ray로 단독(125)에서 거의 안 내려간다. **sparse ray의 지연을 가리는 것은 launch 전체의 ray 수가 아니라 동시에 날아가는 sparse ray의 수**이며, coherent한 base ray는 자기들끼리 메모리를 쓰느라 도움을 주지 못한다.

결론적으로 (b)는 sparse 샘플링의 정의에서 직접 나온다. 8픽셀 간격으로 뽑는 한 워프 내 발산은 피할 수 없고, 샘플을 군집화하면 coverage를 잃는다. **이 5배는 구현이 아니라 구조의 비용이다.**
